...
public class CompraTicketController {
	...
	public ModelAndView saveCompraTicket(@Valid @ModelAttribute("unaCompraTicket") CompraTicket compraTicketNuevo, BindingResult resultado){
		ModelAndView modelView = new ModelAndView();
		if(resultado.hasErrors()){
			GRUPO05.fatal("ERROR DE VALIDACION");
			modelView.setViewName("compraTicket");
			modelView.addObject("unaCompraTicket", compraTicketNuevo);
			return modelView;
		}
		LocalDate ahora = LocalDate.now();
		try {
			compraTicketNuevo.setFechaCompra(ahora);
		...
		modelView.addObject("formUsuarioErroeMessage", "Usuario Guardado correctamente");
		modelView.addObject("unaCompra", compraTicketService.nuevaCompraTicket());
		modelView.setViewName("compraTicket");
		return modelView;
	}
	...
	public ModelAndView ObtenerFormularioEditarTicket(Model model, @PathVariable(name="compraTiket") Integer id) throws Exception {
